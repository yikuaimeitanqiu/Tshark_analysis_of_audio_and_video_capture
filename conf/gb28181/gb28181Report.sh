...
#创建输出报告文件
touch $DIR/report/gb28181report.txt
cat /dev/null > $DIR/report/gb28181report.txt


#打印输入的参数
cat $DIR/report/gb28181report_old.txt > $DIR/report/gb28181report.txt
/usr/bin/rm -rf $DIR/report/gb28181report_old.txt
...
cat $DIR/report/Call_ID.txt >> $DIR/report/gb28181report.txt
/usr/bin/rm -rf $DIR/report/Call_ID.txt

#打印会话产生的RTP端口
printf "\n\n根据 gb28181deriveall.pcap 数据报文中分析出RTP媒体端口信息： \n" >> $DIR/report/gb28181report.txt
cat $DIR/report/rtp.txt >> $DIR/report/gb28181report.txt
/usr/bin/rm -rf $DIR/report/rtp.txt

#打印会话产生的RTCP端口
printf "\n\n根据 gb28181deriveall.pcap 数据报文中分析出RTCP媒体端口信息： \n" >> $DIR/report/gb28181report.txt
cat $DIR/report/rtcp.txt >> $DIR/report/gb28181report.txt
